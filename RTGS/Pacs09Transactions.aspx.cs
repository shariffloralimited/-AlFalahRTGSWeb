...
namespace RTGS
{
    public partial class Pacs09Transactions : System.Web.UI.Page
    {
        ...
        private DataTable GetData()
        {
            ...
            BatchChecksGrid.Columns[1].FooterText = "";
            BatchChecksGrid.Columns[10].FooterText = "";
            if (TypeList.SelectedValue == "0")
            {
                OutwardDB db = new OutwardDB();
                dt = db.GetOutwardSettlementPacs09(BranchID, Ccy, StatusID, SttlmDt);
            }
            else
            {
                InwardDB db = new InwardDB();
                dt = db.GetInwardSettlementPacs09(BranchID, Ccy, StatusID, SttlmDt);
            }
            return dt;
        }
        ...
        protected void ExcelBtn_Click(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                string xlsFileName = "Pacs09_DailyTransaction(" + SettlementDate + ")-D" + System.DateTime.Today.ToString("yyyyMMdd") + "-T" + System.DateTime.Now.ToString("HHmmss") + ".xlsx";
        ...
        {
            PrintPDF("Pacs09_DailyTransReport.pdf");
        }
        ...
        public void SetDoc(Document document, string title, string str, string LogoImage, DataTable dt, string SettlementDate)
        {
            ...
            float[] headerwidths = { 6, 8, 8, 8, 8, 8, 9, 8, 6, 6, 6, 6, 6, 6, 6, 6, 6 };
            ...
            if (TypeList.SelectedValue == "0")
            {
                ...
                datatable.AddCell(new iTextSharp.text.Phrase("Bill Number", fnt));
                datatable.AddCell(new iTextSharp.text.Phrase("LC Number", fnt));
                datatable.AddCell(new iTextSharp.text.Phrase("Party Name", fnt));
                datatable.AddCell(new iTextSharp.text.Phrase("Branch ID", fnt));
                datatable.AddCell(new iTextSharp.text.Phrase("Other Info", fnt));
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    datatable.AddCell(new iTextSharp.text.Phrase((dt.Rows[i]["InstrInfBillNo"]).ToString(), fnt));
                    datatable.AddCell(new iTextSharp.text.Phrase((dt.Rows[i]["InstrInfLcNo"]).ToString(), fnt));
                    datatable.AddCell(new iTextSharp.text.Phrase((dt.Rows[i]["InstrInfPartyName"]).ToString(), fnt));
                    datatable.AddCell(new iTextSharp.text.Phrase((dt.Rows[i]["InstrInfBranchID"]).ToString(), fnt));
                    datatable.AddCell(new iTextSharp.text.Phrase((dt.Rows[i]["InstrInfOtherInfo"]).ToString(), fnt));
